        ...
        return this
            ...
            .then(function(location) {
                ...
                return self
                    ...
                    .then(function() {
                        ...
                        function loadFromMirror() {
                          ...
                          return load('http://fis-cloud.bj.bcebos.com/components/' + name + '/' + self.version + '/component.json?responseContentDisposition=attachment')
                            ...
                            .error(function() {
                              console.log('Load %s@%s component.json from mirror failed.', name, self.version);
                    ...
                    .then(function(body) {
                        if (!body) {
                            throw new Error('`github:'+self.address+'@' +self.version+'` not found.');
                        }
                        ...
                        if (/<title>([^<]+)<\/title>/.test(content)) {
                          throw new Error('Load ' + self.address+'@' +self.version + ' error. Got: ' + RegExp.$1);
                        }
        ...
        function loadFromMirror() {
          ...
          return load('http://fis-cloud.bj.bcebos.com/components/' + name + '/versions.txt?responseContentDisposition=attachment')
            ...
            .then(function(content) {
              ...
              versions = versions.sort(_.compareVersion);
              // console.log(versions);
              return versions;
            })

            .error(function() {
              console.log('Load versions of %s from mirror failed.', name);
              console.log('Fallback to github.');
              return loadFromGithub();
            })
        }
